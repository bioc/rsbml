...
the problems, you will need to catch the signals thrown by the
\texttt{rsbml\_read} function. There are three different types of
signals: \textit{SBMLFatal}, \textit{SBMLError} and
\textit{SBMLWarning}. The following code catches the error
resulting from a non-existent file and prints the message:
<<problems>>=
bad <- tryCatch(rsbml_read("non-existent-file.xml"), 
                SBMLError = function(fatal) print(fatal$msg))
@
