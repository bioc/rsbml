...
the problems, the following returns a data structure describing the 
errors along with their positions in the SBML, if available. Passing 
\texttt{quiet = TRUE} avoids automatic emission of the problems as R 
console messages and warnings.

<<problems>>=
doc <- rsbml_read("non-existent-file.xml", quiet = TRUE)
rsbml_problems(doc)
@
